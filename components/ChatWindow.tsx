    ...
    <>
      <div className={`fisrtPage p-4 md:p-3   rounded-2xl   w-full max-h-[90%] overflow-hidden flex flex-col text-white bccc`}>
        <div className={`startPage absolute top-0 right-0 left-0 bottom-0 w-[100vw] h-[100vh] bg-[#222831]  z-10 flex flex-col justify-center items-center  ${buttonClicked ? 'bc' : ''}`}>
          ...
          <p className='text-white w-[50%] text-center'>Revolutionize your information retrieval experience with our cutting-edge AI chatbot. Seamlessly process PDF documents, extracting answers to your queries in an instant, making knowledge accessible and interactive.</p>
          <button onClick={() => handleClick() } className='bg-[#00ADB5] text-white text-lg  rounded-3xl p-3 px-8 btn'>Get Started</button>
        </div>
        <h1 className="text-3xl md:text-4xl mb-2 ml-auto font-bold  bcccc mr-auto">
          Pdf pedia
        </h1>
        <h3 className="text-xl mb-4 ml-auto mr-auto pt-3">
          HOW TO USE❓
        </h3>
        <ul>
          <li className="text-l">
          ...
          <li className="hidden text-l md:block">
            🌐
            <span className="ml-2">
              2. Download and Set Up Ollama: To harness the full power of PDF-pedia, you'll need to download and set up Ollama. Ollama allows the web app to access a locally running Mistral instance. Follow these steps:
          ...
          <li>
            ⚙️
            <span className="ml-2">
              3. Download Ollama from the official repository.
...
Allow Site Access to Ollama: To enable PDF-pedia to communicate with Ollama, you need to set a few environment variables. Open a terminal and run the following commands:
              ...
              <br/>
              <pre className="inline-flex px-2 py-1 my-2 rounded text-[#FE7A36]">OLLAMA_HOST=127.0.0.1:11435 ollama pull mistral</pre>
            </span>
          </li>
      ...
      <form onSubmit={embedPDF} className="mt-4 flex justify-between items-center w-full">
        <input id="file_input" type="file" accept="pdf" className="text-white rounded-lg" onChange={(e) => e.target.files ? setSelectedPDF(e.target.files[0]) : null}></input>
        <button type="submit" className="shrink-0 px-8 py-3 bg-[#00ADB5] rounded-3xl w-28 ">
    ...
    <>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        <div className="flex w-full mt-4">
          <input
            className="grow mr-8 bg-gray-500 p-3 rounded-xl placeholder:text-gray-400 text-white"
            value={input}
  ...
  return (
    <div className={`flex flex-col items-center p-4 md:p-8 rounded-2xl grow overflow-hidden ${(readyToChat ? "border-none bg-[#393E46] " : "")}`}>
      <h2 className={`${readyToChat ? "" : "hidden"} text-2xl`}>{"📃"} {"Pdf Pedia"}</h2>
      {readyToChat
